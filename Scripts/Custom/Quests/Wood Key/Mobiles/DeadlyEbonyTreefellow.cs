...
namespace Server.Mobiles
{
    ...
    public class DeadlyEbonyTreefellow : BaseCreature
    {
        [Constructable]
        public DeadlyEbonyTreefellow()
            : base(AIType.AI_Mystic, FightMode.Evil, 10, 1, 0.2, 0.4)
        {
            Name = "a Deadly Ebony Treefellow";
            Body = 301;
			Hue = 1457;
            ...
            if (0.3 > Utility.RandomDouble())
                PackItem(new FreshEbonyLog());

            SetWeaponAbility(WeaponAbility.Dismount);
        }

        public DeadlyEbonyTreefellow(Serial serial)
            : base(serial)
        {
        }
