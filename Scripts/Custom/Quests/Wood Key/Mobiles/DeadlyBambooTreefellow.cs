...
namespace Server.Mobiles
{
    ...
    public class DeadlyBambooTreefellow : BaseCreature
    {
        [Constructable]
        public DeadlyBambooTreefellow()
            : base(AIType.AI_Mystic, FightMode.Evil, 10, 1, 0.2, 0.4)
        {
            Name = "a Deadly Bamboo Treefellow";
            Body = 301;
			Hue = 1719;
            ...
            if (0.3 > Utility.RandomDouble())
                PackItem(new FreshBambooLog());

            SetWeaponAbility(WeaponAbility.Dismount);
        }

        public DeadlyBambooTreefellow(Serial serial)
            : base(serial)
        {
        }
