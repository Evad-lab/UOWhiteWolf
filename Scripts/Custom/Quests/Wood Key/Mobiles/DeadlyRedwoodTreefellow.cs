...
namespace Server.Mobiles
{
    ...
    public class DeadlyRedwoodTreefellow : BaseCreature
    {
        [Constructable]
        public DeadlyRedwoodTreefellow()
            : base(AIType.AI_Mystic, FightMode.Evil, 10, 1, 0.2, 0.4)
        {
            Name = "a Deadly Redwood Treefellow";
            Body = 301;
			Hue = 37;
            ...
            if (0.3 > Utility.RandomDouble())
                PackItem(new FreshRedwoodLog());

            SetWeaponAbility(WeaponAbility.Dismount);
        }

        public DeadlyRedwoodTreefellow(Serial serial)
            : base(serial)
        {
        }
