...
namespace Solaris.ItemStore
{
	...
	public class SmallBODListEntry : ItemListEntry 
	{
		...
		public override List<ItemListEntryColumn> Columns
		{
			get
			{
				if( _Columns == null )
				{
					...
					_Columns.Add( new ItemListEntryColumn( 450, "Gem Type", Enum.GetName( typeof( GemType ) , (int)_GemType ) ) );
				}
		...
		public SmallBODListEntry( Item item ) : base ( item )
		{
			...
			if (bod is SmallTinkerBOD)
				_GemType = ((SmallTinkerBOD)bod).GemType;
			else
		...
		public override Item GenerateItem()
		{
			...
			try
			{			
				bod = (SmallBOD)Activator.CreateInstance( _Type, new object[]{ _AmountCur, _AmountMax, _BODFillType, _Number, _Graphic, _RequireExceptional, _Material, 0, _GemType } );
			}
			catch(Exception ex)
			{
				bod = (SmallBOD)Activator.CreateInstance( _Type, new object[]{ _AmountCur, _AmountMax, _BODFillType, _Number, _Graphic, _RequireExceptional, _Material, 0 } );
			}
    ...
    {
        public override int GumpWidth { get { return 800; } }
	...
	{
		public override int GumpWidth{ get{ return 900; } }
		...
		public override List<ItemListEntryColumn> Columns
		{
			get
			{
				if( _Columns == null )
				{
					...
					_Columns.Add( new ItemListEntryColumn( 650, "Gem Type", Enum.GetName( typeof( GemType ), (int)_GemType ) ) );
				}
		...
		public LargeBODListEntry( Item item ) : base ( item )
		{
			...
			if (bod is LargeTinkerBOD)
				_GemType = ((LargeTinkerBOD)bod).GemType;
			else
				_GemType = GemType.None;
		...
		public override Item GenerateItem()
		{
			...
			try
			{
				bod = (LargeBOD)Activator.CreateInstance( _Type, new object[]{ _AmountMax, _RequireExceptional, _Material, _Entries, _GemType } );
			}
			catch(Exception ex)
			{
				bod = (LargeBOD)Activator.CreateInstance( _Type, new object[]{ _AmountMax, _RequireExceptional, _Material, _Entries } );
			}
