...
namespace Server.Mobiles
{
	...
	public class ForsakenBeast : BasePeerless
	{		
	// dreadhorn
		public virtual int StrikingRange{ get{ return 12; } }
	
		[Constructable]
		public ForsakenBeast() : base( AIType.AI_Spellweaving, FightMode.Closest, 10, 1, 0.2, 0.4 )
		{
			Name = "The Forsaken Beast";
			Title = "of the Forgotten Land";
			Body = 256;
			Hue = 1976;
			BaseSoundID = 0xA8;
		...
		public ForsakenBeast( Serial serial ) : base( serial )
		{
		}
		...
		public void HoofStomp()
		{
            ...
            {
                if (m.GetStatMod("ForsakenBeastStr") == null)
                {
                    double percent = m.Skills.MagicResist.Value / 100;
                    int malas = (int)(-20 + (percent * 5.2));

                    m.AddStatMod(new StatMod(StatType.Str, "ForsakenBeastStr", m.Str < Math.Abs(malas) ? m.Str / 2 : malas, TimeSpan.FromSeconds(60)));
                    m.AddStatMod(new StatMod(StatType.Dex, "ForsakenBeastDex", m.Dex < Math.Abs(malas) ? m.Dex / 2 : malas, TimeSpan.FromSeconds(60)));
                    m.AddStatMod(new StatMod(StatType.Int, "ForsakenBeastnt", m.Int < Math.Abs(malas) ? m.Int / 2 : malas, TimeSpan.FromSeconds(60)));
                }

                m.SendLocalizedMessage(1075081); // *ForsakenBeasts eyes light up, his mouth almost a grin, as he slams one hand to the ground!*
            }
        ...
        {
            return m.GetStatMod("ForsakenBeastStr") != null;
        }
