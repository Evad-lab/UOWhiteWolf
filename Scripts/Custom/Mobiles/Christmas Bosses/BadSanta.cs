...
{
	public class BadSanta : BaseCreature
	{
		...
		private List<BadElves> m_Spirits;
		...
		[Constructable]
		public BadSanta() : base( AIType.AI_Mage, FightMode.Closest, 18, 1, 0.2, 0.4 )
		{
			...
			SetSkill( SkillName.Tactics, 106.9, 112.2 );
			SetSkill( SkillName.MagicResist, 90.4, 110.8 );
			SetSkill( SkillName.Magery, 156.2, 161.4 );
			SetSkill( SkillName.EvalInt, 200.0 );
			SetSkill( SkillName.Meditation, 420.0 );

			m_Spirits = new List<BadElves>();
		...
		public override void OnMovement(Mobile m, Point3D oldLocation)
        {
            if (DateTime.Now >= m_NextTalk && InRange(m, 4) && InLOS(m)) // check if it's time to talk & mobile in range & in los.
            {
                ...
                {
                    case 0: Say("You better watch out"); //make it say ...
                    
                        break;
                    case 1: Say("You better not cry");
               
                        break;
                    case 2: Say("Better not pout");
                  
                        break; //
                    case 3: Say("I'm telling you why");
  
                        break;
                    case 4: Say("Santa Claus is coming to town");
  
                        break;
					case 5: Say("He's making a list and checking it twice");
                    
                        break;
					case 6: Say("Gonna find out Who's Naughty and Nice");
                     
                        break;
					case 7: Say("Santa Claus is coming to town");
		...
		//UOWW: commented out for compatiblity with the new Core
		//
		...
		public void Morph()
		{
			...
			Say( 1049499 ); // You've been Naughty!!
			...
			if ( map != null )
			{
				for ( int i = 0; i < m_Offsets.Length; i += 2 )
				{
					...
					BadElves spawn = new BadElves( this );
		...
		public BadSanta( Serial serial ) : base( serial )
		{
			m_Instances.Add( this );
		}
		...
		public override void Serialize( GenericWriter writer )
		{
			...
			writer.WriteMobileList<BadElves>( m_Spirits );
		}
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			switch ( version )
			{
				case 0:
				{
					...
					m_Spirits = reader.ReadStrongMobileList<BadElves>();
	...
	[CorpseName( "a Bad Elf's corpse" )]
	public class BadElves : BaseCreature
	{
		private Mobile m_BadSanta;

		private DrainTimer m_Timer;

		[CommandProperty( AccessLevel.GameMaster )]
		public Mobile BadSanta
		{
			get
			{
				return m_BadSanta;
		...
		[Constructable]
		public BadElves() : this( null )
		{
		}
		
		public BadElves( Mobile BadSanta ) : base( AIType.AI_Melee, FightMode.Closest, 10, 1, 0.2, 0.4 )
		{
			m_BadSanta = BadSanta;
       ...
       public override void OnDamage(int amount, Mobile from, bool willKill)
		...
		public BadElves( Serial serial ) : base( serial )
		{
		}
		...
		{
			private BadElves m_Owner;

			public DrainTimer( BadElves owner ) : base( TimeSpan.FromSeconds( 5.0 ), TimeSpan.FromSeconds( 5.0 ) )
			{
				m_Owner = owner;
				Priority = TimerPriority.TwoFiftyMS;
			}
			...
			protected override void OnTick()
			{
				...
				{
					if ( m == m_Owner || m == m_Owner.BadSanta || !m_Owner.CanBeHarmful( m ) )
						continue;
				...
				foreach ( Mobile m in m_ToDrain )
				{
					...
					if ( m_Owner.BadSanta != null )
						m_Owner.BadSanta.Hits += drain;
