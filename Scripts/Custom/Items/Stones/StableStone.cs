...
public class StableStone : Item
	{ 
			...
			protected override void OnTarget( Mobile from, object targeted )
			{
				if ( targeted is BaseCreature )
					m_Trainer.EndStable( from, (BaseCreature)targeted );
						...
						from.SendMessage ("Sorry, the stable stone isnt an inn");
				else
		...
		public void BeginStable( Mobile from )
		{
			...
			{
				from.SendMessage ("You can't stable that. You have too many pets stabled");
			}
			else
			{
				from.SendMessage ("The stable stone charges 100 Gold");
		...
		public void EndStable( Mobile from, BaseCreature pet )
		{
			...
			else if ( pet.Body.IsHuman && !(pet is Mercenary))
			{
				from.SendMessage ("You do not own that pet");
			}
			...
			{
				from.SendMessage ("Your pet seems to be busy at the moment, try again when it's not");
			}
			...
			else
			{
				...
				if ((from != null && from.Backpack != null && from.Backpack.ConsumeTotal(typeof(Gold), 100)) || Banker.Withdraw(from, 100))
				{
