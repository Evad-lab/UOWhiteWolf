	...
	public class LevelItemTarget : Target 
	{
		...
		protected override void OnTarget( Mobile from, object target )
		{
			if ( target is BaseWeapon || target is BaseArmor || target is BaseClothing || target is BaseJewel )
			{
				...
				{		
					if (target != null && ((BaseWeapon)target).Crafter != null)
					{
						...
						from.SendMessage("You can only make this weapon levelable if it was crafted");
				}
				else
					from.SendMessage("You can only make weapons levelable");
			}
			else
			{
